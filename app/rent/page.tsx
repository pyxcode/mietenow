...
export default function RentPage() {
  ...
  const [formData, setFormData] = useState({
    minPrice: preferences.min_price?.toString() || '500',
    maxPrice: preferences.max_price?.toString() || '1500',
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-end">
                  <button 
                    ...
                    disabled={loading}
                    className="w-full bg-[#004AAD] hover:bg-[#002E73] text-white px-6 py-3 rounded-lg text-lg font-semibold transition-colors duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Search className="w-5 h-5" />
                    {loading ? (language === 'de' ? 'Speichern...' : 'Saving...') : (language === 'de' ? 'Weiter' : 'Continue')}
                  </button>
                </div>
              </div>
            </div>

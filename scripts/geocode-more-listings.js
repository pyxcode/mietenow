...
async function geocodeMoreListings() {
  await connectDB()
  ...
  for (const listing of listingsWithoutCoords) {
    try {
      ...
      let addressToGeocode = 'Berlin, Germany'
      
      // Si le titre contient des informations de localisation, l'utiliser
      if (listing.title) {
        // Chercher des quartiers de Berlin dans le titre
        const berlinDistricts = [
          'kreuzberg', 'prenzlauer berg', 'friedrichshain', 'neukölln', 
          'charlottenburg', 'schöneberg', 'tempelhof', 'steglitz', 
          'zehlendorf', 'spandau', 'reinickendorf', 'pankow', 
          'weissensee', 'lichtenberg', 'treptow', 'köpenick', 
          'moabit', 'wedding', 'mitte', 'tiergarten'
        ]
        
        const titleLower = listing.title.toLowerCase()
        for (const district of berlinDistricts) {
          if (titleLower.includes(district)) {
            addressToGeocode = `${district}, Berlin, Germany`
            break
          }
        }
      }
  ...
  console.log(`✅ ${successCount} nouvelles annonces géocodées`)
  
  await mongoose.disconnect()
  console.log('\n✅ Déconnecté de MongoDB')
}

geocodeMoreListings().catch(console.error)
